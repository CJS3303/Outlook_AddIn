...
namespace OutlookAddIn1
{
  ...
  <contextMenus>
    <contextMenu idMso='ContextMenuCalendarItem'>
      ...
      <button id='CancelIgnore' label='Cancel Ignore' onAction='OnCancelIgnore'/>
    </contextMenu>
  </contextMenus>";
...
<customUI xmlns='http://schemas.microsoft.com/office/2009/07/customui' onLoad='OnRibbonLoad'>
  ...
  {ExplorerContextMenusXml()}
    ...
    MessageBoxIcon.Information);
        ...
        // === CANCEL IGNORE (UN-IGNORE) ===
        public async void OnCancelIgnore(Office.IRibbonControl control)
        {
            Outlook.Explorer exp = null;
            Outlook.Inspector insp = null;
            Outlook.AppointmentItem appt = null;
            ...
            try
            {
                ...
                if (appt == null)
                {
                    MessageBox.Show("No calendar item selected.", "Cancel Ignore");
                    return;
                }

                var email = GetCurrentUserEmailAddress();

                // ✅ CRITICAL FIX: Use BOTH GlobalId and EntryId to find the record
                // When a meeting is ignored BEFORE being sent, it's stored with EntryId
                // When we try to cancel ignore AFTER it's sent, it now has a GlobalAppointmentID
                var globalIdFromAppt = GetGlobalId(appt);
                var entryIdFromAppt = appt.EntryID ?? "";

                var tempRec = new MeetingRecord
                {
                    GlobalId = globalIdFromAppt,
                    EntryId = entryIdFromAppt,
                    StartUtc = appt.StartUTC,
                    UserDisplayName = email
                };

                // ✅ ENHANCED DEBUG: Log what we're looking for
                System.Diagnostics.Debug.WriteLine($"OnCancelIgnore: Looking for ignored timesheet:");
                System.Diagnostics.Debug.WriteLine($"  GlobalId (from appt): '{globalIdFromAppt}'");
                System.Diagnostics.Debug.WriteLine($"  EntryId (from appt): '{entryIdFromAppt}'");
                System.Diagnostics.Debug.WriteLine($"  StartUtc: {tempRec.StartUtc:yyyy-MM-dd HH:mm:ss}");
                System.Diagnostics.Debug.WriteLine($"  StartTorontoTime: {tempRec.StartTorontoTime:yyyy-MM-dd HH:mm:ss}");
                System.Diagnostics.Debug.WriteLine($"  UserDisplayName: {email}");

                // ✅ FIX: Try BOTH IDs - first with GlobalId, then with EntryId if not found
                var existingTimesheet = await DbWriter.GetExistingTimesheetAsync(tempRec);

                if (existingTimesheet == null && !string.IsNullOrWhiteSpace(entryIdFromAppt))
                {
                    // Try again with EntryId as the global_id
                    System.Diagnostics.Debug.WriteLine($"OnCancelIgnore: Not found with GlobalId, trying with EntryId as global_id...");
                    tempRec.GlobalId = entryIdFromAppt; // Override GlobalId with EntryId
                    existingTimesheet = await DbWriter.GetExistingTimesheetAsync(tempRec);
                }

                // ✅ ENHANCED DEBUG: Log what we found
                if (existingTimesheet == null)
                {
                    System.Diagnostics.Debug.WriteLine($"OnCancelIgnore: ❌ No record found in database (tried both GlobalId and EntryId)");
                    MessageBox.Show(
                        "No timesheet record found for this meeting.\n\n" +
                        "The meeting may have already been un-ignored or was never ignored.",
                        "Cancel Ignore");
                    return;
                }

                System.Diagnostics.Debug.WriteLine($"OnCancelIgnore: ✅ Found record with status='{existingTimesheet.Status}'");

                // Verify it's actually ignored
                if (!string.Equals(existingTimesheet.Status, "ignored", StringComparison.OrdinalIgnoreCase))
                {
                    MessageBox.Show(
                        $"This meeting is not in ignored status.\n\n" +
                        $"Current status: {existingTimesheet.Status ?? "(null)"}\n\n" +
                        $"Only meetings with 'ignored' status can be un-ignored.",
                        "Cancel Ignore");
                    return;
                }
                ...
                var dialogResult = MessageBox.Show(
                    $"Remove ignore status from this meeting?\n\n" +
                    $"Subject: {appt.Subject}\n" +
                    $"Date: {appt.Start:MMM dd, yyyy HH:mm}\n\n" +
                    $"This will allow it to appear in the unsubmitted list again.",
                    "Cancel Ignore",
                    MessageBoxButtons.YesNo,
                    MessageBoxIcon.Question);
                ...
                // ✅ FIX: Pass the FOUND record (with correct GlobalId) to delete
                var deleted = await DbWriter.CancelIgnoreTimesheetAsync(existingTimesheet);

                if (!deleted)
                {
                    MessageBox.Show("Failed to remove ignored status from database.", "Error");
                    return;
                }

                System.Diagnostics.Debug.WriteLine($"OnCancelIgnore: Successfully deleted ignored record");

                // ✅ FIX: COMPLETELY REMOVE "Timesheet Ignored" category (restore to default/no category)
                try
                {
                    var categoryName = "Timesheet Ignored";

                    System.Diagnostics.Debug.WriteLine($"OnCancelIgnore: Removing category from appt.Categories='{appt.Categories}'");

                    if (!string.IsNullOrEmpty(appt.Categories))
                    {
                        // Remove BOTH timesheet categories and keep only other categories
                        var categories = appt.Categories.Split(new[] { ',', ';' }, StringSplitOptions.RemoveEmptyEntries)
                            .Select(c => c.Trim())
                            .Where(c => !c.Equals("Timesheet Submitted", StringComparison.OrdinalIgnoreCase) &&
                                       !c.Equals("Timesheet Ignored", StringComparison.OrdinalIgnoreCase))
                        ...
                        // ✅ Set to empty string if no other categories remain (restore default appearance)
                        appt.Categories = categories.Count > 0
                            ? string.Join(", ", categories)
                            : string.Empty;

                        appt.Save();
                        System.Diagnostics.Debug.WriteLine($"OnCancelIgnore: Updated categories to '{appt.Categories}' (empty = default appearance)");
                    }
                    else
                    {
                        System.Diagnostics.Debug.WriteLine($"OnCancelIgnore: No categories to remove");
                    }
                }
                catch (Exception catEx)
                {
                    System.Diagnostics.Debug.WriteLine($"Failed to remove ignored category: {catEx.Message}");
                }

                MessageBox.Show(
                    "Ignored status removed successfully!\n\n" +
                    "This meeting will now appear in the Unsubmitted list.",
                    "Cancel Ignore Complete");
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"OnCancelIgnore failed: {ex.Message}");
                System.Diagnostics.Debug.WriteLine($"Stack trace: {ex.StackTrace}");
                MessageBox.Show($"Cancel Ignore failed: " + ex.Message, "Error");
            }
            finally
            {
                if (appt != null)
